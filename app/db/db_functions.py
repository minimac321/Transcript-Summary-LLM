    ...
    with sqlite3.connect('MASTER.db') as conn:
        ...
        query = """
            INSERT INTO Transcripts (
                user_id, file_name, transcription, transcription_summary, 
                sentiment_label, sentiment_report, prev_ai_research, fact_check
            ) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """
...
def insert_audio(file_path, transcript_id):
    with sqlite3.connect('MASTER.db') as conn:
        with open(file_path, 'rb') as f:
            audio_data = f.read()
        conn.execute("""
            INSERT INTO AudioFiles (file_name, audio_data, transcript_id, user_id) 
            VALUES (?, ?, ?, ?)
